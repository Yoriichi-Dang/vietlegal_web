...
export default function ProjectGrid({
  ...
  const [selectedChatItem, setSelectedChatItem] = useState<ProjectItem | null>(
    null
  );
  ...
  const filteredProjects = useMemo(() => {
    return projects
      .filter((project) => {
        ...
        // Tab filter - Cập nhật logic này
        let matchesTab = true;

        if (activeTab === "recent") {
          matchesTab = true; // Hiển thị tất cả
        } else if (activeTab === "starred") {
          matchesTab = project.starred;
        } else if (activeTab === "shared") {
          matchesTab = project.shared;
        } else if (activeTab === "trash") {
          matchesTab = false; // Implement trash logic
        } else if (activeTab === "thuế") {
          // Danh mục thuế
          matchesTab =
            project.tags?.includes("thuế") ||
            project.tags?.includes("doanh nghiệp") ||
            project.name.toLowerCase().includes("thuế") ||
            project.icon === "Calculator";
        } else if (activeTab === "bảo hiểm") {
          // Danh mục bảo hiểm
          matchesTab =
            project.tags?.includes("bảo hiểm") ||
            project.tags?.includes("xã hội") ||
            project.name.toLowerCase().includes("bảo hiểm") ||
            project.icon === "Shield";
        } else if (activeTab === "pháp lý") {
          // Danh mục pháp lý
          matchesTab =
            project.tags?.includes("luật") ||
            project.tags?.includes("pháp lý") ||
            project.name.toLowerCase().includes("luật") ||
            project.icon === "Scale";
        } else if (activeTab === "hồ-sơ-thuế") {
          // Bộ sưu tập hồ sơ thuế
          matchesTab =
            project.tags?.includes("thuế") ||
            project.name.toLowerCase().includes("thuế") ||
            project.type === "folder";
        } else if (activeTab === "tài-liệu-bảo-hiểm") {
          // Bộ sưu tập tài liệu bảo hiểm
          matchesTab =
            project.tags?.includes("bảo hiểm") ||
            project.name.toLowerCase().includes("bảo hiểm");
        } else if (activeTab === "tư-vấn-pháp-lý") {
          // Bộ sưu tập tư vấn pháp lý
          matchesTab =
            project.tags?.includes("tư vấn") ||
            project.tags?.includes("pháp lý") ||
            project.name.toLowerCase().includes("tư vấn");
        } else if (activeTab === "mẫu-văn-bản") {
          // Bộ sưu tập mẫu văn bản
          matchesTab =
            project.tags?.includes("mẫu đơn") ||
            project.name.toLowerCase().includes("mẫu") ||
            project.fileType === "word";
        }
  ...
  const handleChatWithItem = (item: ProjectItem, e: React.MouseEvent) => {
    e.stopPropagation();
                ...
                >
                  ...
                  <div className="col-span-1">
                    <div className="flex items-center space-x-1">
                      <button
                        onClick={(e) => handleChatWithItem(project, e)}
              ...
              <div className="aspect-video bg-neutral-700 rounded-t-lg flex items-center justify-center relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity">
                  <div className="flex items-center space-x-1">
                    <button
                      onClick={(e) => handleChatWithItem(project, e)}
                      className="p-1 bg-neutral-800/80 rounded text-neutral-300 hover:text-blue-400"
                      title="Chat với AI"
                    >
                      <MessageCircle className="w-4 h-4" />
                    </button>
